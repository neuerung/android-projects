...
public class CaptureActivity extends DecoderActivity {
	...
	public void onCreate(Bundle icicle) {
		...
		try {
			...
			if(b != null)
			{
				...
				fromScreen = b.getString("fromScreen");
	...
	private void handleDecodeInternally(Result rawResult, ResultHandler resultHandler, Bitmap barcode) {
		...
		Content = displayContents.toString();

		if(fromScreen.equals("client"))
		{//If Capture Activity is called from Client Screen(Splash0), get the event list for captured client
			new Splash0Thread(this, "GetEventList", Content).execute();
		}

		else if(fromScreen.equals("event"))
		{
	...
	public void ShowEventScreen(ArrayList<SplashOWrapper> _objEventList)
	{
		...
		{
			Intent intent = new Intent();
			intent.putExtra("content", "Not a valid Client");
			setResult(RESULT_OK, intent);
			finish();
		}
	}
