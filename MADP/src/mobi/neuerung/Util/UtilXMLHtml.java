...
public class UtilXMLHtml {
	...
	public void SetHtmlPageFromXML(ScreenPartWrapper _objScreenPartWrapper, String Section , final Context context, String ScreenNumber)
	{
		...
		String PageName = objElementWrapper.Title.trim();


//		StringBuilder sb = new StringBuilder("<html><head><link rel='stylesheet' type='text/css' href='http://www.robotsidekick.com/test.css?rev=0' /></head><body><h1>Hello World</h1></body></html>");
		StringBuilder sb = new StringBuilder(PageName);
		PageName= sb.toString();
		if(PageName.length()>0) 
		{
			WView.loadData(URLEncoder.encode(PageName).replaceAll("\\+", " "), "text/html", Encoding.UTF_8.toString());
